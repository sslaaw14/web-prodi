...
<body>
  <div class="container-scroller">
    ...
    $query  = "SELECT k.ke_id, k.tanggal, k.judul, k.deskri, g.p_nama FROM kegiatan k, galery g WHERE k.p_id = g.p_id order by k.ke_id desc LIMIT $offset, $rowsPerPage";
       $result = mysqli_query($conn, $query) or die('Error, query failed'); 
     $query2   = "SELECT COUNT(ke_id) AS numrows FROM kegiatan";
                    ...
                    <table class="table table-hover">
                      ...
                      <tbody>
                        ...
                        <tr>
                          <td><?= $hasil['judul'] ?></td>
						   <td><?= $hasil['deskri'] ?></td>
                           <td><?= jumlahkomentar($hasil['ke_id']) ?></td>
                          <td><?= $hasil['tanggal'] ?></td>
                          <td><a href="updatekegiatan.php?id=<?= $hasil['ke_id'] ?>"><button type="button" class="btn btn-light"><i class="menu-icon mdi mdi-table-edit"></i></button></a> </td>
                          <td><a href="hapusArtikel.php?id=<?= $hasil['ke_id'] ?>" onclick="return confirm('Yakin Ingin menghapus?')"><button type="button" class="btn btn-danger"><i class="menu-icon mdi mdi-delete"></i></button></a></td>
                        </tr>
                          <?php } ?>
                      </tbody>  
<?php
		     ...
		     <a href="newkegiatan.php"><button type="button" class="btn btn-icons btn-inversed btn-success"><i class="menu-icon mdi  mdi-message-text-outline"></i></button></a>
                  </div>
                </div>
              </div>
            </div>
