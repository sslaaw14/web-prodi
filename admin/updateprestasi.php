...
include "config.php";
    $id= $_GET['updt'];
$query   = "SELECT judul,deskri,tanggal FROM prestasi WHERE pre_id ='$id'";
	$result = mysqli_query($conn, $query) or die('Error, query failed. ' . mysqli_error($conn));
	$hasil = mysqli_fetch_array($result);
    list($judul,$deskripsi,$tgl) = $hasil;
if(isset($_POST['upload']))
{    

    $judul = $_POST['judul'];
    $deskri = $_POST['deskripsi'];
	$query = "UPDATE prestasi SET judul='$judul',deskri='$deskri' WHERE pre_id=$id";
                ...
                <div class="card-body">
                  ...
                  <form class="forms-sample" action="" method="post">
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" id="exampleInputName1" placeholder="Nama Kegiatan" name="judul" value="<?php echo $judul?>">
                    </div>
                    <div class="form-group">
                      <label for="exampleTextarea1">Deskripsi</label>
                      <textarea class="form-control" id="exampleTextarea1" rows="10" name="deskripsi"><?php echo $deskripsi?></textarea>
                    </div>
                    <button type="submit" class="btn btn-success mr-2" name="upload">Simpan</button>
                  </form>
                </div>
              </div>
            </div>
